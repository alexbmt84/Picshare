// Add a like to a comment function...

function deleteComment(commentId) { // We pass the comment id as argument...

    fetch(`/comment/${commentId}`, { // Try to connect to /comment/:commentid route...

        method: 'POST', // Using method POST...
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ commentId })

    })

    .then(response => { // If we have a response...

        if (response.ok) { // If response is ok...

            // Comment is correctly sent to the route...
            // Refresh the page to update the UI
            location.reload();

        } else if (response.status === 401) { // Cannot send data to the route...

            // Handle like error

        }else if (response.status === 403) {
            console.log("Vous n'êtes pas autorisé à supprimer ce commentaire.");
        } else {
            console.log("Une erreur s'est produite lors de la suppression du commentaire.");
        }

    })

    .catch(error => { // Error while trying to connect to the route...

        // Optimize error message
        console.log(error);
        // Handle like error

    });

}
